...
def main():
    ...
    # Finn riktig enhet (GPU/CPU)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    
    # Initier Inference direkte på riktig device for å unngå tensor-konflikt
    ...
    for jsonl_key in processed_files:
        ...
        for speaker, segments in speaker_segments.items():
            ...
            for start, end, dur in top_segments:
                ...
                try:
                    # Inference kjører nå på GPU automatisk fordi vi satt device=device i konstruktøren
